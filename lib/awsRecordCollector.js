...
function fetchAndMergeAllResults(sourceSchema, paramSet, callback) {
  ...
  function fetchAndMergeAll(collector, err, res) {
    ...
    if (_.get(res, sourceSchema.incompleteIndicator)) {
      currentParams = sourceSchema.nextBatchParamConstructor(_.cloneDeep(currentParams), res);
      return api[sourceSchema.apiMethod](currentParams, _.partial(fetchAndMergeAll, collector));
    } else {
      return callback(err, collector);
    }
  }
  return api[sourceSchema.apiMethod](currentParams, _.partial(fetchAndMergeAll, null));
}
