...
function getApi(sourceSchema, paramSet) {
  ...
  const apiConfig = paramSet.apiConfig;
  ...
  if (!host) {
    throw new Error(`Host not found in k8s params ${paramSet} for call to ${sourceSchema.name}`);
  }
  if (!sourceSchema.apiMethod) {
    throw new Error(`Path not found in k8s sourceSchema ${sourceSchema}`);
  }
  const urlBuilder = _.template('https://${host}' + (_.startsWith(sourceSchema.apiMethod, '/') ? sourceSchema.apiMethod : '/' + sourceSchema.apiMethod));
