...
const widgetHelpers = {
  ...
  textBox: {
    builder: ({screen, grid, schema, setInput, getInput}) => {
      ...
      input.setValue(getInput(schema.inputKeys || ''));
      screen.render();
      input.on('submit', () => {
  ...
  jsonArea: {
    builder: ({screen, grid, schema, setInput, getInput, send}) => {
      ...
      input.on('blur', () => {
        try {
          json = JSON.parse(input.getValue());
  ...
  messages: {
    builder: ({screen, grid, schema, setInput, getInput, subscribe}) => {
      ...
      function addMessage(message) {
        ...
        input.setContent(lines.join('\n'));
        screen.render();
      }
      input.key(['up'], () => input.scroll(-1));
      input.key(['down'], () => input.scroll(1));
...
function buildWidget({screen, grid, gopher, schema, name, client})  {
  ...
  function update(err, data) {
    ...
    widget = widget || widgetHelpers[schema.displayType].builder({screen, grid, schema, data: widgetData, setInput: gopher.setInput, getInput: gopher.getInput, inputFields});
    widgetHelpers[schema.displayType].updater({widget, data: widgetData, metadata, inputFields});
    if (schema.inputKeys && inputElements.indexOf(widget) === -1) {
...
function widgetDashboard({dataDependencies, inputs, display}) {
  ...
  _.each(display.widgets, (schema, name) => {
    widgets.push(buildWidget({
      screen, grid, gopher, schema, name, client: passer.client(schema.title)
    }));
  });
}
